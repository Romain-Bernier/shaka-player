...
shaka.text.VttTextParser = class {
  ...
  static parseCue_(text, timeOffset, regions) {
    ...
    // Get color info and delete it from payload
    // Match only the pattern <c.color>text</c>
    // If not leave the extra information
    const payloadSplit = payload.split('\n');
    const regex = /.*(<c\.([a-zA-Z]+)>)(.*(\n.*)?)(<\/c>).*/;
    let i;
    let cuecolor;
    for (i = 0; i < payloadSplit.length; i++) {
      if (regex.test(payloadSplit[i])) {
        const group = regex.exec(payloadSplit[i]);
        cuecolor = String(group[2]);
        if (['white', 'lime', 'cyan', 'red', 'yellow',
          'magenta', 'blue', 'black', 'green'].includes(cuecolor)) {
          payloadSplit[i] = group[3];
        } else {
          cuecolor = 'white';
        }
      }
    }
    payload = payloadSplit.join('\n');
