...
shaka.text.VttTextParser = class {
  ...
  parseMedia(data, time) {
    ...
    for (let i = 1; i < lines.length; i++) {
      if (/^Region:/.test(lines[i])) {
        const region = VttTextParser.parseRegion_(lines[i]);
        regions.push(region);
      }
    }
    ...
    const ret = [];
    if (!/-->($|[ \t\n])/m.test(str)) {
      shaka.log.debug('empty vtt segment')
    ...
    // Parse cues.
    if (/-->($|[ \t\n])/m.test(str)) {
    shaka.log.debug('segment vtt with subtitles');
      for (let i = 1; i < blocks.length; i++) {
        lines = blocks[i].split('\n');
        const cue = VttTextParser.parseCue_(lines, offset, regions);
        const regex = /.*(\<c\.(?<color>[a-zA-Z]+)\>)(?<text>.*(\n.*)?)(\<\/c\>).*/;
        if (regex.test(lines)) {
          const rcolor = regex.exec(lines)
          cue.color = String(rcolor[2]);
        }
  ...
  static parseCue_(text, timeOffset, regions) {
    ...
    const regex = /.*(\<c\.(?<color>[a-zA-Z]+)\>)(?<text>.*(\n.*)?)(\<\/c\>).*/;
    let i;
    for (i = 0; i < payload_split.length; i++) { 
      if (regex.test(payload_split[i])) {
        let text = regex.exec(payload_split[i])
        payload_split[i] = text[3];
      }
    }
    payload = payload_split.join('\n');

    shaka.log.debug('text', text.slice(1).join('\n'));
    const cue = new shaka.text.Cue(start, end, payload);
    shaka.log.debug('start', start, 'end', end, 'payload', payload);
  ...
  static parseTime_(parser) {
    ...
    const miliseconds = Number(results[4]);
